...
async def track_attack_progression(events: list[dict]) -> dict:
    ...
    for idx, event in enumerate(events):
        ...
        best_stage   = max(event_scores, key=event_scores.get)
    ...
    for stage_key in STAGE_ORDER:
        ...
        progression_matrix.append({
            ...
            "stage":        stage_key,
            "name":         s["name"],
            "status":       "OBSERVED" if stage_activity[stage_key] else "NOT_OBSERVED",
            "soc_priority": s["soc_priority"],
        })

    # Prioritized response actions based on highest stage
